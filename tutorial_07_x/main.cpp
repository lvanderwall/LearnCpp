...
int binarySearch(int *array, int target, int min, int max)
{
    ...
    int cen{0};
    while(min <= max) {                             // need to check "min == max"
        cen = min + (max - min) / 2;                // use integer division to round down, avoid overflow
        if      (target > array[cen]) min = cen + 1;// +1/-1: no need to check cen again
        else if (target < array[cen]) max = cen - 1;
        else return cen;
    }
